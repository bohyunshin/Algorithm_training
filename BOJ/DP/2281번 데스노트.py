    ...
    for k in range(m+1):
        ...
        if dp[j][k] != 1e100:
            ...
            if k + 1 + people[j+1] <= m:
                ...
                if dp[j+1][k+1+people[j+1]] > dp[j][k] - (m-k)**2 + (m - (k+1+people[j+1]))**2:
                    dp[j+1][k+1+people[j+1]] = dp[j][k] - (m-k)**2 + (m - (k+1+people[j+1]))**2
                # i번째 줄에 j+1번째 사람을 쓰지 않고 i+1번째 줄에 쓰거나,
                if dp[j+1][people[j+1]] > dp[j][k] + (m - people[j+1])**2:
                    dp[j+1][people[j+1]] = dp[j][k] + (m - people[j+1])**2
            # 크다면, 무조건 다음 줄에 써야 함.
            else:
